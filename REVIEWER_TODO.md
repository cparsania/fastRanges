...
Ordered from highest to lowest priority, based on what most directly
affects Bioconductor acceptance and how much the current package
contract differs from `findOverlaps()`.

## Priority 1: Core Compatibility / Semantics

Add `select=` support to
...
- Target parity with Bioconductor-style values:
  ...
  - `last`
  - `arbitrary`
- Default should remain `select = "all"` so current `Hits` behavior is
  preserved.
- Added tests against
...
Fix overlap behavior involving empty ranges
...
Handle circular sequences safely
...
Confirm whether default single-thread performance is slower on some
systems
...
Extend randomized validation to cover `select=`

- `first` / `last` checked for exact identity with Bioconductor.
- `arbitrary` checked for semantic validity against the full reference
  hit set.
