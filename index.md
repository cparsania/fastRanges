...
[![Bioconductor](https://img.shields.io/badge/Bioconductor-published-green)](https://bioconductor.org/packages/release/bioc/html/fastRanges.html)
...
`fastRanges` is designed to stay close to Bioconductor overlap semantics
for supported inputs, but it is currently best viewed as a
high-throughput engine for `IRanges` and `GRanges`, not as a blanket
...
- Use `deterministic = FALSE` when maximum multithreaded throughput
  matters more than stable hit ordering.
